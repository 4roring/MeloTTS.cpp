...
/**
 * @class Korean
 * @brief 한국어 TTS를 위한 언어 모듈 클래스
 */
class Korean : public AbstractLanguageModule {
public:
    Korean(std::unique_ptr<ov::Core>& core_ptr, const std::filesystem::path& data_folder);
    virtual ~Korean() = default;

    // 텍스트를 음소/음절 단위로 변환
    virtual std::tuple<std::vector<std::string>, std::vector<int64_t>, std::vector<int>>
    g2p(const std::string& segment, std::shared_ptr<OpenVinoTokenizer>& tokenizer) override;
